...
// HMAC-SHA512 实现（严格遵循 SLIP-0010）
fn hmac_sha512(key: &[u8], data: &[u8]) -> [u8; 64] {
    let mut hmac = HmacSha512::new_from_slice(key).expect("HMAC key should be valid length");
    hmac.update(data);
    hmac.finalize().into_bytes().into()
}

/// SLIP-0010 密钥派生（逐步骤调试）
fn slip0010_derive_ed25519(seed: &[u8], path: &str) -> SigningKey {
    // 初始化 HMAC
    ...
    for segment in path.split('/').skip(1) {
        let index = parse_segment(segment).expect("Invalid segment");
    ...
    SigningKey::from_bytes(&key[..32].try_into().unwrap())
}

/// 地址生成（严格遵循规范）
fn generate_sui_address(pub_key: &VerifyingKey) -> String {
    // 输入格式：0x00 || 32字节公钥
    let mut hasher_input = [0u8; 33];
    hasher_input[0] = ED25519_FLAG;
    hasher_input[1..].copy_from_slice(pub_key.as_bytes());
...
/// 路径解析（带硬化标记处理）
fn parse_segment(segment: &str) -> Result<u32, &'static str> {
...
/// 主函数（完整路径构造）
pub fn get_sui_address(mnemonic: &str) -> String {
    // 1. 生成 BIP-39 种子（空密码）
    let mnemonic = Mnemonic::<English>::new_from_phrase(mnemonic).expect("Invalid mnemonic phrase");
    ...
    // 4. 生成地址
    generate_sui_address(&pub_key)
}

#[test]
fn test_official_vector() {
    let address = get_sui_address(
...
fn test_pub_key() {
    ...
    // 步骤 1：Base64 解码
    let pub_key_bytes = base64::decode(base64_pub_key).unwrap();
...
/// 从原始公钥字节生成地址（关键验证函数）
fn generate_sui_address_from_bytes(pub_key: &[u8]) -> String {
    let mut hasher_input = [0u8; 33];
